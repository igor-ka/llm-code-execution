...
<!--
Adapted from agent-skills by Addy Osmani (MIT), upstream commit d2478bf.
See ../NOTICE.md. Cross-references to skills that were not vendored here have
been repointed or removed so this skill stands alone.
-->
...
This app's threat model is unusual: **it runs model-generated code on purpose.** The boundaries
that actually matter here:

| Boundary | Untrusted input | Existing control |
| --- | --- | --- |
| Browser → `/api/execute` | prompt text | Zod validation; OIDC bearer gate (`auth.ts`) requiring the `execute:code` scope |
| Claude → backend | **generated code + the `should_execute` verdict** | never `eval`'d in-process; language allowlisted server-side; only ever runs inside the sandbox |
| Backend → sandbox | the generated program | `--network none`, read-only rootfs, all capabilities dropped, `no-new-privileges`, non-root user, pids/memory/CPU caps, wall-clock timeout, `--rm` |
| Browser → `/api/sessions`, `/api/runs` | session/run ids, search strings | owner taken from the verified `sub`; every query filtered on owner; non-owned → **404** (no enumeration leak); parameterized SQL; LIKE wildcards escaped |
| Host → backend | — | the Docker socket is mounted, which is root-equivalent control of the host — **local dev only**, see the README's *Security posture* |
