...
```bash
# 5-6. The two that CHANGE ON EVERY REBUILD, both read from Terraform rather than typed.
cd infra
...
**Those last two are why this section is a runbook and not a one-time note.** `random_password.db`
is regenerated by every `terraform apply` into a fresh project, and the Valkey PSC endpoint is
newly allocated on every rebuild — so the values that worked yesterday are wrong today. Both are
read from `terraform output` rather than retyped, which is also why neither has to be written
down anywhere.

`redis://`, not `rediss://`: the Valkey endpoint is a private PSC address inside the project VPC,
